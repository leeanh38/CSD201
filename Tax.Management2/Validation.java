...
public class Validation {
    ...
    public double checkInputDoublePositive(double bottom){
        while(true){
            try{
    ...
    public double checkInputDoubleSmaller(double top){
        while(true){
    ...
    public int checkInputNodeLimit(LinkedList list, int min, int max){
        //Case 1: list rỗng
        if(min != 0 && max == 0){
        ...
        while(true){
            try{
                int result = Integer.parseInt(sc.nextLine().trim());
                if(result<min || result > list.size()) 
                    throw new NumberFormatException();
                return result;
            }
            catch(NumberFormatException e){
